...
// Categories
export const listCategories = () => getJSON<Category[]>("/servicios/categorias");
...
export const listServices = (q?: string, categoryId?: number) => {
  ...
  return getJSON<Service[]>(`/servicios?${params.toString()}`);
};

export const getService = (id: number) => getJSON<Service>(`/servicios/${id}`);
...
export const createService = (payload: ServiceCreate) => postJSON<Service>("/servicios/", payload);

export const updateService = (id: number, payload: Partial<ServiceCreate>) => putJSON<Service>(`/servicios/${id}`, payload);

export const deactivateService = async (id: number) => {
  const res = await fetch(`${import.meta.env.VITE_API_URL ?? 'http://127.0.0.1:8000'}/servicios/${id}`, {
    method: 'DELETE',
...
// Images
export const setServiceImages = (serviceId: number, images: ServiceImage[]) => postJSON<ServiceImage[]>(`/servicios/${serviceId}/imagenes`, images);
export const listServiceImages = (serviceId: number) => getJSON<ServiceImage[]>(`/servicios/${serviceId}/imagenes`);
...
export const listMyServices = (active?: boolean) => {
  ...
  return getJSON<Service[]>(`/servicios/mios?${params.toString()}`);
};
